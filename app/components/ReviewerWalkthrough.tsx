...
import { ArrowLeft, ArrowRight, Check, LoaderCircle, Pause, Play, RotateCcw, X } from "lucide-react";
...
export default function ReviewerWalkthrough({ open, loading, toolEvents, error, onClose, onRetry, onOpenEvidence }: ReviewerWalkthroughProps) {
  const [elapsedSeconds, setElapsedSeconds] = useState(0);
  const [playing, setPlaying] = useState(true);
  const closeButtonRef = useRef<HTMLButtonElement>(null);
  const data = useMemo(() => buildReviewerWalkthrough(toolEvents ?? []), [toolEvents]);
  ...
  useEffect(() => {
    if (!open || loading || !data || error || !playing || complete) return;
    const timer = window.setInterval(() => {
      setElapsedSeconds((current) => Math.min(REVIEWER_WALKTHROUGH_SECONDS, current + 1));
    }, 1000);
    return () => window.clearInterval(timer);
  }, [complete, data, error, loading, open, playing]);

  const moveToStage = (nextIndex: number, pause = true) => {
  ...
  }, [data, loading, onClose, open, stageIndex]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-0 backdrop-blur-sm sm:p-4" role="presentation">
      <section role="dialog" aria-modal="true" aria-labelledby="reviewer-walkthrough-title" className="flex h-[100dvh] w-full flex-col overflow-hidden border-white/10 bg-ink-950 shadow-2xl sm:h-[min(820px,calc(100dvh-2rem))] sm:max-w-6xl sm:rounded-md sm:border">
        <header className="shrink-0 border-b border-white/10">
          <div className="flex items-start justify-between gap-4 px-4 py-3 sm:px-5">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500"><span>Reviewer walkthrough</span><span aria-hidden="true">/</span><span>Chapter {stageIndex + 1} of {reviewerStages.length}</span></div>
              <h2 id="reviewer-walkthrough-title" className="mt-1 truncate text-base font-semibold text-white sm:text-lg">ConsultIQ capability review</h2>
            </div>
            <div className="flex shrink-0 items-center gap-2">
              {data ? <button type="button" onClick={onOpenEvidence} className="hidden min-h-9 items-center rounded border border-white/10 px-3 text-xs text-slate-300 hover:bg-white/10 sm:inline-flex">Full evidence</button> : null}
              <button ref={closeButtonRef} type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center rounded border border-white/10 text-slate-400 hover:bg-white/10 hover:text-white" aria-label="Close reviewer walkthrough"><X size={17} /></button>
            </div>
          </div>
        ...
        {loading ? (
          <div className="flex flex-1 items-center justify-center p-6" aria-live="polite">
            <div className="text-center"><LoaderCircle size={28} className="mx-auto animate-spin text-emerald-200" /><h3 className="mt-4 text-lg font-semibold text-white">Running the governed workflow</h3><p className="mt-2 max-w-md text-sm leading-6 text-slate-400">Loading the fake source packet, executing bounded tools, checking compliance, and preparing reviewer evidence.</p></div>
          </div>
        ) : error || !data ? (
          <div className="flex flex-1 items-center justify-center p-6" aria-live="assertive">
            <div className="max-w-md text-center"><h3 className="text-lg font-semibold text-white">The walkthrough could not load</h3><p className="mt-2 text-sm leading-6 text-slate-400">{error ?? "The workflow response did not contain the structured evidence required for this review."}</p><button type="button" onClick={onRetry} className="mt-5 inline-flex min-h-10 items-center gap-2 rounded border border-emerald-300/30 bg-emerald-300/10 px-4 text-sm text-emerald-50 hover:bg-emerald-300/20"><RotateCcw size={15} />Run again</button></div>
          </div>
        ) : (
          <>
            <div className="grid min-h-0 flex-1 lg:grid-cols-[210px_1fr]">
              <nav aria-label="Reviewer walkthrough chapters" className="shrink-0 overflow-x-auto border-b border-white/10 lg:overflow-y-auto lg:border-b-0 lg:border-r">
                <ol className="flex min-w-max p-2 lg:min-w-0 lg:flex-col lg:p-3">
                  {reviewerStages.map((item, index) => {
                    ...
                    return <li key={item.id}><button type="button" onClick={() => moveToStage(index)} aria-current={active ? "step" : undefined} className={`flex min-h-11 w-full items-center gap-3 rounded px-3 py-2 text-left text-xs transition-colors ${active ? "bg-white/10 text-white" : "text-slate-500 hover:bg-white/[0.05] hover:text-slate-300"}`}><span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border text-[11px] ${visited ? "border-emerald-300/30 bg-emerald-300/10 text-emerald-100" : active ? "border-sky-300/35 text-sky-100" : "border-white/10"}`}>{visited ? <Check size={12} /> : index + 1}</span><span className="whitespace-nowrap lg:whitespace-normal">{item.shortLabel}</span></button></li>;
                  })}
                </ol>
              </nav>
              <main className="min-h-0 overflow-y-auto px-4 py-5 sm:px-6 lg:px-8 lg:py-6">
                <div className="mx-auto max-w-4xl">
                  ...
                  <div key={stage.id} className="animate-fade-in-up"><ReviewerStageContent stageIndex={stageIndex} data={data} /></div>
                </div>
              </main>
            </div>
            ...
            <footer className="shrink-0 border-t border-white/10 bg-ink-900 px-4 py-3 sm:px-5">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex flex-1 items-center justify-center gap-2">
                  ...
                  <button type="button" onClick={() => { if (complete) { setElapsedSeconds(0); setPlaying(true); } else { setPlaying((current) => !current); } }} className="inline-flex min-h-10 min-w-28 items-center justify-center gap-2 rounded border border-emerald-300/25 bg-emerald-300/10 px-4 text-sm text-emerald-50 hover:bg-emerald-300/20">{complete ? <RotateCcw size={15} /> : playing ? <Pause size={15} /> : <Play size={15} />}{complete ? "Replay" : playing ? "Pause" : "Resume"}</button>
